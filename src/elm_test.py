...
def save_top10_models(model, _index):
  for k in range(10):
    ...
    with open('../models/elm_model_TOP'+str(k+1)+'.json', "w") as json_file:
      json_file.write(model_json)
    # serialize weights to HDF5
    model[_index[k]].save_weights('../models/elm_model_TOP'+str(k)+'.h5')
  print("Saved models to disk")

# Find neural network config for Top 10
def find_top10config(score, top10, u_acti):
  table = PrettyTable(['Top', 'u1', 'MSE'])
  k=0
  for k in range(10):
    i=0
...
def save_top10plot(history, _index):
  ...
  for k in range(10):
    ...
    # ax.set_yscale('log')
    #plt.show()
    plt.savefig('../imgs/ab/elm_TOP'+str(k+1)+'_ab.png')
...
def main():
  ...
  for u1 in u_acti:
    ...
    # Debug
    print ("Model: ", u1, nodes)
    print("Progress: %.2f%%" % (i/float(len(u_acti)*1)*100))
    #print(model.summary())
    #print(model.layers())
    i=i+1
  ...
  table = find_top10config(net_score, top10, u_acti)
  save_top10(table)
