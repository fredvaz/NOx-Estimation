...
def build_network(mode, hidden_layer_nodes, f1, f2, w1, b1, w2, b2):
  if mode=='LMB':
    train=True
  elif mode=='ELM':
    train=False
  ...
  model.add(Dense(hidden_layer_nodes, input_dim=5,
                  activation=f1, kernel_initializer=w1, bias_initializer=b1,
                  use_bias=True, trainable=train ))

  # Output layer: Output Variable
  model.add(Dense(1,
                  activation=f2, kernel_initializer=w2, bias_initializer=b2,
                  use_bias=True, trainable=train ))
      ...
      for b1 in b_init:
        for w2 in w_init:
          for b2 in b_init:
            ...
            model = build_network('ELM', hidden_layer_nodes, f1, f2, w1, b1, w2, b2)
