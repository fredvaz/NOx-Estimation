...
# Load TOP 10 models
def load_top10_models(k):
  # load json and create model
  json_file = open('../models/model_TOP'+str(k)+'.json', 'r')
  loaded_model_json = json_file.read()
  json_file.close()
  loaded_model = model_from_json(loaded_model_json)
  # load weights into new model
  loaded_model.load_weights('../models/model_TOP'+str(k)+'.h5')
  print '\nLoaded TOP'+str(k)+' model from disk'
  return loaded_model

# Save TOP1 plot on a .png file
def save_plot_test_predection(y_test, y_predictions, _str, k):
...
def main():
  ...
  for k in range(1,11):
    ...
    save_plot_test_predection(y_test, _y, 'test', k)
    net_score.append(score)
    y_predictions.append(_y)
  # Score Mean TOP10
  net_score_mean = numpy.mean(net_score)
  print '\n\nTEST MSE MEAN: ' + str(net_score_mean) +'\n\n'
  # Desired vs Predection TOP10 Mean
  y_predictions_mean = numpy.mean(y_predictions)
  # Plot Mean
  save_plot_test_predection(y_test, y_predictions_mean, 'test_mean', k)
  ...
  for k in range(1,11):
    ...
    save_plot_test_predection(y_train, _y, 'train', k)
    net_score.append(score)
    y_predictions.append(_y)
  # Score Mean TOP10
  net_score_mean = numpy.mean(net_score)
  ...
  y_predictions_mean = numpy.mean(y_predictions)
  # Plot Mean
  save_plot_test_predection(y_test, y_predictions_mean, 'train_mean', k)
