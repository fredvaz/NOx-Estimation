  ...
  for b1 in w_b_init:
    for w2 in w_b_init:
      for b2 in w_b_init:
        for f1 in f_acti:
          for f2 in f_acti:
            #for n in deep_net_size:
            model = Sequential()

            #Input Variables: Input Layer, Hidden Layers
            model.add(Dense(hidden_layer1_nodes, input_dim=5,
                            kernel_initializer=w1, bias_initializer=b1, activation=f1 ))
            model.add(Dense(hidden_layer2_nodes,
                            kernel_initializer=w2, bias_initializer=b2, activation=f2 ))
            model.add(Dense(hidden_layer3_nodes,
                            kernel_initializer=w3, bias_initializer=b3, activation=f3 ))

            # Output layer: Output Variable
            model.add(Dense(1,
                            kernel_initializer=w4, bias_initializer=b4, activation=f4 ))
